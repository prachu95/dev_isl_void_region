...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname ver_40) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
#|
Proposal :
...
(define left_obj_starting_posn (make-posn (- 0 object_width) (/ world-height 2)))

; Right object starting postion
(define right_obj_starting_posn (make-posn (+ world-width object_width) (/ world-height 2)))
...
(check-expect (tick (make-world 1
                                (make-Shape 1 (make-posn (+ 100 world-width) 300) "red")
                                (make-Shape 2 (make-posn world-width 300) "yellow")
                                #true))
              (make-world 1
                          (make-Shape 1 left_obj_starting_posn "red")
...
(check-expect (void_region_calc (make-world 1
                                                   ...
                                                   (make-Shape 2 (make-posn 1200 300) "yellow")
                                                   #true))
              (+ (* (* 1 void_region_ramp_up) void_region_width) void_region_width))

(check-expect (void_region_calc (make-world 10
                                                   (make-Shape 1 (make-posn 0 300) "red")
                                                   (make-Shape 2 (make-posn 1200 300) "yellow")
                                                   #true))
              (+ (* (* 10 void_region_ramp_up) void_region_width) void_region_width))

(check-expect (void_region_calc (make-world 100
                                                   (make-Shape 1 (make-posn 0 300) "red")
                                                   (make-Shape 2 (make-posn 1200 300) "yellow")
                                                   #true))
              (+ (* (* 100 void_region_ramp_up) void_region_width) void_region_width))
...
(define (color_selector option)
  (cond
    ...
    [(= 3 option) "blue"]
    [(= 4 option) "orange"]
    [(= 5 option) "darkmagenta"]
    [(= 6 option) "green"]
    [(= 7 option) "gray"]
    [(= 8 option) "lightseagreen"]
    [(= 9 option) "slateblue"]
    [(= 10 option) "tan"]))
...
(check-expect (color_selector 5) "darkmagenta")
(check-expect (color_selector 6) "green")
(check-expect (color_selector 7) "gray")
(check-expect (color_selector 8) "lightseagreen")
(check-expect (color_selector 9) "slateblue")
(check-expect (color_selector 10) "tan")
...
(define (shape_drawer type colour)
  (cond
    ...
    [(= type 1) (circle (/ object_width 2) "solid" colour)]
    [(= type 2) (square object_width "solid" colour)]
    [(= type 3) (triangle object_width "solid" colour)]
    [(= type 4) (rhombus object_width 45 "solid" colour)]
    [(= type 5) (star object_width "solid" colour)]
    [(= type 6) (regular-polygon object_width 5 "solid" colour)]
    [(= type 7) (regular-polygon object_width 6 "solid" colour)]
    [(= type 8) (right-triangle object_width object_height "solid" colour)]
    [(= type 9) (regular-polygon object_width 7 "solid" colour)]
...
(check-expect (shape_drawer 1 "yellow")
              (circle (/ object_width 2) "solid" "yellow"))

(check-expect (shape_drawer 2 "tan")
              (square object_width "solid" "tan"))

(check-expect (shape_drawer 3 "white")
              (triangle object_width "solid" "white"))

(check-expect (shape_drawer 4 "gray")
              (rhombus object_width 45 "solid" "gray"))

(check-expect (shape_drawer 5 "lightgreen")
              (star object_width "solid" "lightgreen"))

(check-expect (shape_drawer 6 "seagreen")
              (regular-polygon object_width 5 "solid" "seagreen"))

(check-expect (shape_drawer 7 "seagreen")
              (regular-polygon object_width 6 "solid" "seagreen"))

(check-expect (shape_drawer 8 "blue")
              (right-triangle object_width object_height "solid" "blue"))

(check-expect (shape_drawer 9 "seagreen")
              (regular-polygon object_width 7 "solid" "seagreen"))

(check-expect (shape_drawer 18 "blue")
...
(check-expect (intersection_checker (make-world 1
                                                (make-Shape 1 (make-posn (/ world-width 2) 300) "red")
                                                (make-Shape 2 (make-posn (/ world-width 2) 300) "yellow")
                                                #true))
              #true)
...
; Called by the "key" event, when a button is clicked
...
(check-expect (status_check (make-world 1
                                        (make-Shape 4 (make-posn (/ world-width 2) 300) "blue")
                                        (make-Shape 6 (make-posn (/ world-width 2) 300) "white")
                                        #true)
                            1 1 2 2)
              (make-world 2
                          (make-Shape 1 left_obj_starting_posn "red")
...
(check-expect (status_check (make-world 1
                                        (make-Shape 4 left_obj_starting_posn "blue")
                                        (make-Shape 6 right_obj_starting_posn "white")
                                        #true)
                            1 1 2 2)
              (make-world 1
                          (make-Shape 1 left_obj_starting_posn "red")
...
(check-expect (draw_level (make-world 1
                                      (make-Shape 0  left_obj_starting_posn "red")
                                      (make-Shape 18 right_obj_starting_posn "yellow")
                                      #true))
              (place-image (text (number->string 1) 50 "black")
                           level_disp_x level_disp_y
                           (place-image (rectangle (+ (* (* 1 void_region_ramp_up) void_region_width) void_region_width) world-height "solid" "LightSlateGray")
                                        (- initial_void_region_x (/ 2 (+ (* (* 1 void_region_ramp_up) void_region_width) void_region_width))) initial_void_region_y
                                        (place-image (ellipse object_width object_height "solid" "red")
                                                     (posn-x left_obj_starting_posn) (posn-y left_obj_starting_posn)
                                                     (place-image (rectangle object_width object_height "solid" "yellow")
                                                                  (posn-x right_obj_starting_posn) (posn-y right_obj_starting_posn)
                                                                  EMPTY)))))
...
(check-expect (draw_void_region (make-world 10
                                            ...
                                            (make-Shape 18 right_obj_starting_posn "yellow")
                                            #true))
              (place-image (rectangle (+ (* (* 10 void_region_ramp_up) void_region_width) void_region_width) world-height "solid" "LightSlateGray")
                           (- initial_void_region_x (/ 2 (+ (* (* 10 void_region_ramp_up) void_region_width) void_region_width))) initial_void_region_y
                           (place-image (ellipse object_width object_height "solid" "red")
                                        (posn-x left_obj_starting_posn) (posn-y left_obj_starting_posn)
                                        (place-image (rectangle object_width object_height "solid" "yellow")
                                                     (posn-x right_obj_starting_posn) (posn-y right_obj_starting_posn)
...
(check-expect (draw_object_left (make-world 1
                                            (make-Shape 0 (make-posn 0 300) "red")
                                            (make-Shape 18 (make-posn 1200 300) "yellow")
                                            #true))
              (place-image (ellipse object_width object_height "solid" "red")
...
(check-expect (draw_object_left (make-world 1
                                            (make-Shape 18 (make-posn 0 300) "red")
                                            (make-Shape 18 (make-posn 1200 300) "yellow")
                                            #true))
