...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname ver_16) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
#|
Proposal :
...
--> Don't forget to reset the initial positions in "tick" when it reaches the end. 
...
(define (tick level)
  (cond
    ...
    [(<= (posn-x world-left_img-pos) 0)             (make-world world-level
                                                                (make-Shape world-left_img-type  left_obj_starting_posn  world-left_img-colour)
                                                                (make-Shape world-right_img-type right_obj_starting_posn world-left_img-colour)
                                                                world-current_game_status
                                                                world-final_game_status)]
    ...
    [else                                           (make-world world-level
                                                                (make-Shape world-left_img-type  left_obj_posn_updater(world-left_img-pos)  world-left_img-colour)
                                                                (make-Shape world-right_img-type right_obj_posn_updater(world-right_img-pos) world-left_img-colour)
                                                                world-current_game_status
                                                                world-final_game_status)]))
...
; left_obj_posn_updater : posn -> posn
...
(define (left_obj_posn_updater pos))
...
(define (right_obj_posn_updater pos))
