...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname ver_19) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
#|
Proposal :
...
(define (common_posn_updater pos level symbol)
  (make-posn (symbol (posn-x pos) const_obj_pos_update (* object_speed_ramp_up level)) (posn-y pos)))
...
(define (left_obj_posn_updater pos level) (common_posn_updater pos level +))
...
(define (right_obj_posn_updater pos level) (common_posn_updater pos level -))
...
(define (tick world)
  (cond
    [(>= (posn-x (Shape-pos (world-left_img world))) (world-width)) (make-world world-level
                                                                (make-Shape world-left_img-type  left_obj_starting_posn  world-left_img-colour)
                                                                (make-Shape world-right_img-type right_obj_starting_posn world-left_img-colour)
                                                                world-current_game_status
                                                                world-final_game_status)]

    [(<= (posn-x (world-right_img-pos world) 0))             (make-world world-level
                                                                (make-Shape world-left_img-type  left_obj_starting_posn  world-left_img-colour)
                                                                (make-Shape world-right_img-type right_obj_starting_posn world-left_img-colour)
                                                                world-current_game_status
                                                                world-final_game_status)]

    [else                                           (make-world world-level
                                                                (make-Shape world-left_img-type  left_obj_posn_updater(world-left_img-pos world-level)  world-left_img-colour)
                                                                (make-Shape world-right_img-type right_obj_posn_updater(world-right_img-pos world-level) world-left_img-colour)
                                                                world-current_game_status
                                                                world-final_game_status)])) 
...
(define (win_game world)
  (place-image (text "You Won the Game...!!!" 25 "black")
...
; Check "final_game_status" and stops
; if true. 
...
(define (draw_void_region world)
  (cond
    [(>= (+ (* (* world-level void_region_ramp_up) void_region_width) void_region_width) world-width) (win_game world)]
    [else (place-image (rectangle (+ (* (* world-level void_region_ramp_up) void_region_width) void_region_width) void_region_height "solid" "LightSlateGray")
                (- initial_void_region_x (/ 2 (+ (* (* world-level void_region_ramp_up) void_region_width) void_region_width))) initial_void_region_y
